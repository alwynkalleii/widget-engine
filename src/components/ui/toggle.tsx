...
function Toggle(props: ComponentProps<typeof TogglePrimitive> & VariantProps<typeof toggleVariants>) {
  const [local, others] = splitProps(props, ["class", "variant", "size"])

  return (
    <TogglePrimitive
      data-slot="toggle"
      class={cn(toggleVariants({ variant: local.variant, size: local.size, class: local.class }))}
      {...others}
    />
  )
}
